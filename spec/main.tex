...
$\E$ denotes an elliptic-curve group of prime order\footnote{The curve order is denoted by {\tt curve}\_{\tt order} in the code.}~$q$ with generator $G$. %(Note we write $\E$ additively, whereas multiplicative notation is used in~\cite{cggmp24}.)
...
The security parameters of the protocol are denoted collectively by $L = (\kappa, \varepsilon, \ell, \ell', m)$.
...
Some of the zero-knowledge proofs have privacy loss and soundness error at least~$2^{-\ell}$, but since $\ell = \lambda \geq s$ that is fine.
    ...
    \item The curve order $q$ and parameter $m$ determine the soundness error of several of the zero-knowledge proofs, with some of the proofs having soundness error at least~$1/q$ and others having soundness error at least~$2^{-m}$.
    For 128-bit security, using a curve with $\lambda \geq 256$ and setting $m=128$ suffices.
...
The following protocol is based on \cite[Figure~7]{cggmp24}, although we have corrected some typos and eliminated some extraneous parts. (In particular, we do not have identifiable abort.)
